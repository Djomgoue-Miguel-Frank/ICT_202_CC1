...
export interface CvUpsertInput {
  fullName: string;
  headline: string;
  groupRole: string;
  matricule: string;
...
const DEFAULT_SEED_DATA: SeedCvData[] = [
  {
    ...
    phone: "+237 657560037",
    profile:
      "Je suis etudiant a l'Universite de Yaounde I, faculte des sciences, filiere ICT4D, specialite securite des systemes informatiques.",
  ...
  },
  {
    ...
    phone: "+237 654187578",
    profile:
      "Etudiant a l'Universite de Yaounde I, passionne par le developpement web, les systemes informatiques et les nouvelles technologies.",
  ...
  },
  {
    ...
    phone: "+237 693172961",
    profile:
      "Je suis etudiant passionne par la cyber-securite, le developpement web, le developpement mobile, les technologies et l'innovation.",
  ...
  },
  {
    ...
    phone: "+237 659949883",
    profile:
      "Etudiant passionne par la securite informatique, le developpement logiciel et le design graphique (UI/UX).",
  ...
  },
  {
    ...
    phone: "",
    profile:
      "Etudiante de 19 ans a l'Universite de Yaounde 1. Formation en securite informatique, reseau informatique, marketing digital et communication.",
  ...
  },
  {
    ...
    phone: "+237 657751373",
    profile:
      "Etudiant en Licence 2 ICT4D, passionne par les technologies et le numerique. Rigoureux, organise et dote d'un bon esprit d'equipe.",
    hobbies:
      "Jeux, programmation, sciences, technologie, histoire, philosophie, art, litterature, sports, musique et voyages.",
    languages: ["Francais", "Anglais"],
    education: [
  ...
  },
  {
    ...
    phone: "+237 655520015",
    profile:
      "Je suis etudiante a l'Universite de Yaounde I, en cycle licence ICT4D specialite Genie logiciel.",
  ...
  },
  {
    ...
    phone: "+237 658174874",
    profile:
      "Etudiant en deuxieme annee ICT4D a l'Universite de Yaounde I, specialite Genie logiciel. Passionne par le developpement d'applications et la creation de solutions numeriques.",
...
function buildMediaRecord(cvId: string, kind: MediaKind, file: Blob, fileName: string): MediaRecord {
  const now = Date.now();
  return {
    id: generateId(kind),
    cvId,
...
function sanitizeInput(input: CvUpsertInput): Omit<CvRecord, "id" | "photoAssetId" | "videoAssetId" | "audioAssetId" | "displayOrder" | "createdAt" | "updatedAt"> {
  return {
...
async function fetchLegacyMedia(path?: string): Promise<Blob | null> {
  if (!path) {
    return null;
  }
...
async function prepareSeedData(): Promise<PreparedSeedCv[]> {
  const preparedEntries: PreparedSeedCv[] = [];
  const now = Date.now();
...
export async function getAllCvs(): Promise<CvRecord[]> {
  ...
  const tx = db.transaction(CVS_STORE, "readonly");
  const cvStore = tx.objectStore(CVS_STORE);
  const rows = await requestToPromise(cvStore.getAll() as IDBRequest<CvRecord[]>);
  await transactionToPromise(tx);
...
export async function getCvById(id: string): Promise<CvRecord | null> {
  ...
  const tx = db.transaction(CVS_STORE, "readonly");
  const cvStore = tx.objectStore(CVS_STORE);
  const record = await requestToPromise(cvStore.get(id) as IDBRequest<CvRecord | undefined>);
  await transactionToPromise(tx);
  return record ?? null;
}
...
export async function getMediaBlob(assetId: string): Promise<Blob | null> {
  ...
  const media = await requestToPromise(mediaStore.get(assetId) as IDBRequest<MediaRecord | undefined>);
  await transactionToPromise(tx);
  return media?.blob ?? null;
}

export async function createCv(input: CvUpsertInput): Promise<CvRecord> {
...
export async function updateCv(id: string, input: CvUpsertInput): Promise<CvRecord> {
